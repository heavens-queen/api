  ...
  try {
    ...
    const processedImages = await Promise.all(
      files.map(async (file) => {
        ...
        const thumbnailBuffer = await sharpInstance.resize(100, 100, { fit: sharp.fit.inside,withoutEnlargement:true }).toBuffer();
...
const compressionApplied = ((1 - finalFileSize / OriginalFileSize) * 100).toFixed(2); // Calculate compression as a percentage
