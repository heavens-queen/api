...
export const uploadVideo = async (req, res, next) => {
    try {
        ...
        const thumnailUrl = await uploadThumbnailToAWS(bucketName, thumnailKey, generateThumbnail);
        const compressedFilePath = await compressVideo(tempFilePath);
        const compressedFileSize = (fs.statSync(compressedFilePath).size /
            1024 /
            1024).toFixed(2);
        const url = await uploadVideoFileToAWS(bucketName, objectKey, compressedFilePath);
        const originalFileSize = ((req.file?.size ?? 0) / (1024 * 1024)).toFixed(2);
        const compressionPercentage = Math.round((100 - (Number(compressedFileSize) / Number(originalFileSize)) * 100) *
            100) / 100;
